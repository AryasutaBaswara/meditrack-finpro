...
Deno.test("Handler should reject incomplete payloads", async () => {
    const mockRequest = new Request("http://localhost/functions/v1/stock-webhook", {
        ...
        body: JSON.stringify({
            table: "drugs",
            type: "UPDATE",
            record: { name: "Paracetamol" } // Kurang 'id' dan 'stock'
        })
    });
...
Deno.test("Handler should ignore stock values above threshold", async () => {
    const mockRequest = new Request("http://localhost/functions/v1/stock-webhook", {
        ...
        body: JSON.stringify({
            table: "drugs",
            type: "UPDATE",
            record: { id: "123", name: "Paracetamol", stock: 100 }, // Stok aman
    ...
    const data = await response.json();
    // Jika tidak kirim broadcast, dia masuk ke bagian bawah handler
