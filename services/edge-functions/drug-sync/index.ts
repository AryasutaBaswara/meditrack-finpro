...
const ES_URL = Deno.env.get("ELASTICSEARCH_URL") || "http://localhost:9200";
...
serve(async (req) => {
  try {
    ...
    if (type === "DELETE") {
      ...
      return new Response(JSON.stringify({ status: "deleted", code: res.status }), { status: 200 });
    }
    ...
    const errorMessage = (error as Error)?.message || "Terjadi kesalahan internal";
